use crate::{graph_permutation8::Swap, util, FOURTEEN};
...
impl GraphPermutation14 {
    ...
    pub(crate) const fn calculate_unchecked(mut arr: &mut [u8]) -> Option<Self> {
        let mut swaps = Swap::array_n();
    ...
    pub const fn apply_to_slice<T>(self, arr: &mut [T]) {
        let mut swaps = SwapsIter14::new(&self);
        let mut index = 0;
        while let Some(swap) = swaps.next_const() {
    ...
    pub const fn get_array(&self) -> [u8; FOURTEEN] {
        let mut arr = const { util::indexes_array() };
...
mod test {
    ...
    pub fn test_sequence_for_5_elements() {
        ...
        let arr: [u8; 5] = crate::util::indexes_array();
    ...
    pub fn test_combine() {
        ...
        assert_eq!(
            perm1_then_2.get_array(),
            [6, 0, 2, 4, 1, 3, 5, 7, 8, 9, 10, 11, 12, 13]
        );

        let perm2_then_1 = perm2.combine(&perm1);

        assert_eq!(
            perm2_then_1.get_array(),
            [3, 1, 4, 6, 0, 2, 5, 7, 8, 9, 10, 11, 12, 13]
        );
    }
    ...
    pub fn test_move_element() {
        let mut perm = GraphPermutation14::IDENTITY;
        ...
        assert_eq!(
            perm.get_array(),
            [1, 0, 5, 2, 3, 4, 6, 7, 8, 9, 10, 11, 12, 13]
        );
        perm = perm.with_element_moved(4, 10);
        assert_eq!(
            perm.get_array(),
            [1, 0, 5, 2, 4, 6, 7, 8, 9, 10, 3, 11, 12, 13]
        );
    }
}
